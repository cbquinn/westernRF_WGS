...
# this script pulls in all simulated SFS into a single data.frame (or list) for bringing into analyze with abc
# saves as R objects
...
read_jsfs <- function(model_run, n=10000){
    myfile <- paste0(model_run, "/jsfs_",  model_run, ".txt")
    z <- read.table(myfile)
    z[1:n,]
}
...
# for now remove iso.bot_RMLAS because still running
# REMEMBER TO REMOVE!!!!!!!!!!!!!!!!!!!!!!!
#files.params <- grep("iso.bot_RMLAS", files.params, value=TRUE, invert=TRUE)

# subset these params files to only those that ran to completion 
...
nrow(param.df) == length(inputs$file)*(10000)

# pull these jsfs
setwd("../out")
jsfs.list <- lapply(inputs$model_run, function(x){read_jsfs(x, n = (inputs$lines[inputs$model_run==x])-1)}) 
names(jsfs.list) <- inputs$model_run
jsfs.df <- bind_rows(jsfs.list, .id="model_run")

# check that the two correspond
if(sum(param.df$model_run != jsfs.df$model_run)==0){
...
save(sfs.sim, file = "sfs.sim.RData")
